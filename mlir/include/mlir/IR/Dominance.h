...
/// region-kind specific concept of dominance.  See
/// RegionKindInterface
...
public:
  ...
  /// Return true if the specified block is reachable from the entry
  /// block of its region.  In an SSACFG region, a block is reachable
  /// from the entry block if it is the successor of the entry block
  /// or another reachable block.  In a Graph region, all blocks are
  /// reachable.
  bool isReachableFromEntry(Block *a) const {
  ...
  /// Return true if value A properly dominates operation B. i.e if
  /// the operation that defines A properlyDominates B and the
  /// operation that defines A does not contain B.
  bool properlyDominates(Value a, Operation *b) const;

  /// Return true if operation A dominates operation B. i.e if
  /// the operation that defines A dominates B.
  bool dominates(Value a, Operation *b) const {
    return (Operation *)a.getDefiningOp() == b || properlyDominates(a, b);
  }

  /// Return true if the specified block A dominates block B.  i.e. if
  /// block A and block B are the same block or block A properly
  /// dominates block B.
  ...
  /// Return true if the specified block A properly dominates block B.
  /// i.e.: if block A contains block B, or if the region which
  /// contains block A also contains block B or some parent of block B
  /// and block A dominates that block in that kind of region.  In an
  /// SSACFG region, block A dominates block B if all control flow
  /// paths from the entry block to block B flow through block A.  In
  /// a Graph region, all blocks dominate all other blocks.
  bool properlyDominates(Block *a, Block *b) const {
